...
func save() error {
	...
	if err != nil {
		fmt.Println("Cannot save data to", DATAFILE)
		return err
	}
...
func listAll(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Listing the contents fo the KV store!")
...
func changeElement(w http.ResponseWriter, r *http.Request) {
	fmt.Println("Changing an element in the KV store!")
...
func insertElement(w http.ResponseWriter, r *http.Request) {
	...
	if !ADD(key, n) {
		fmt.Println("Insertion operation failed!")
	} else {
		err := save()
		if err != nil {
			fmt.Println("Failed to save changes:", err)
